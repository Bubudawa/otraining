...
function register_teacher_form() {
	...
	wp_nonce_field( 'create-' . $_SERVER['REMOTE_ADDR'], 'teacher-front', false );
...
add_action( 'admin_post_nopriv_nouvel_utilisateur', 'add_teacher' );
function add_teacher() {
	// Vérifier le nonce (et n'exécuter l'action que s'il est valide)
	if( isset( $_POST['teacher-front'] ) && wp_verify_nonce( $_POST['teacher-front'], 'create-' . $_SERVER['REMOTE_ADDR'] ) ) {
...
add_action( 'wp_footer', 'show_teacher_registration_message' );
function show_teacher_registration_message() {
